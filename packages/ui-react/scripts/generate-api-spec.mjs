...
const uiRoot = path.resolve(packageRoot, '../ui');
const sdkTypes = path.join(sdkRoot, 'lib/types/index.d.ts');
const uiTypes = path.join(uiRoot, 'lib/index.d.ts');
...
function ensureUiTypes() {
    if (fs.existsSync(uiTypes)) {
        return;
    }

    run('pnpm', ['build'], uiRoot);
}
